...
router.get('/ko/complex', async (req, res, next) => {
	...
	const restaurant = await getDataFromDB(`SELECT DISTINCT Url.RNumber, KName, Grade, Phone_Num, Homepage, Price, Image1, Image2, Image3, Cat_Kor as Category FROM Michelin_Kor, Michelin_Location, Url, Dish_Category Where Michelin_Kor.RNumber = Michelin_Location.RNumber AND Url.RNumber = Michelin_Kor.RNumber AND Url.RNumber = Dish_Category.RNumber AND Cat_Kor LIKE "%${req.query['cat']}%" AND Location_Kor LIKE "%${req.query['loc']}%" AND Price >= ${req.query['min'] * 1} AND Price <= ${req.query['max'] * 1} AND Grade = ${req.query['grade'] * 1} GROUP BY Url.RNumber`);
	res.status(200).json(restaurant);     
    }  catch(err) {
        res.status(400).json({message: err.message});
    }
...
router.get('/en/complex', async (req, res, next) => {
	...
	console.log("max " + req.query['max']);
	console.log("grade " + req.query['grade']);
	const restaurant = await getDataFromDB(`SELECT DISTINCT Url.RNumber, EName, Grade, Phone_Num, Homepage, Price, Image1, Image2, Image3, Cat_Eng as Category FROM Michelin_Eng, Michelin_Location, Url, Dish_Category Where Michelin_Eng.RNumber = Michelin_Location.RNumber AND Url.RNumber = Michelin_Eng.RNumber AND Url.RNumber = Dish_Category.RNumber AND Cat_Eng LIKE "%${req.query['cat']}%" AND Location_Eng LIKE "%${req.query['loc']}%" AND Price >= ${req.query['min'] * 1} AND Price <= ${req.query['max'] * 1} AND Grade = ${req.query['grade'] * 1} GROUP BY Url.RNumber`);
	res.status(200).json(restaurant);     
    }  catch(err) {
        res.status(400).json({message: err.message});
    }
